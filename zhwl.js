...
async function artic() {
	...
	let sign = sha256_Encrypt(_data);
	let options = { method: 'Get', url: `${hostname}/api/article/channel_list?channel_id=5bd2bef6b1985042e7691870&is_new=1&list_count=10&size=10&start=${ts}`, headers: { 'X-SESSION-ID': ck[0], 'X-REQUEST-ID': ck[1], 'X-TIMESTAMP': ts, 'X-SIGNATURE': sign, 'Cache-Control': `no-cache`, 'X-TENANT-ID': `3`, 'Host': host, }, };
...
async function read(id) {
	...
	let sign = sha256_Encrypt(_data);
	let options = { method: 'Get', url: `${hostname}/api/article/detail?from_channel=5bd2bef6b1985042e7691870&id=${id}`, headers: { 'X-SESSION-ID': ck[0], 'X-REQUEST-ID': ck[1], 'X-TIMESTAMP': ts, 'X-SIGNATURE': sign, 'Cache-Control': `no-cache`, 'X-TENANT-ID': `3`, 'Host': host, }, };
